...
- **2台虚拟机使用Internal网络模式连接，其中一台虚拟机上配置DHCP服务，另一台服务器作为DHCP客户端，从该DHCP服务器获取网络地址配置**
  ...
  - 在目标主机上运行脚本 `dhcp.sh`，`sudo netplan apply`
    ![](img/dhcp_cli.PNG)
  - 在工作主机netplan配置文件中添加enp0s9，`sudo netplan apply`。这时可以`ifconfig`查看到新增加的网卡。
    ![](img/dhcp_ifc.png)

---

### 5. Samba
- 配置文件：[smb.conf](config/smb.conf)
- 在linux端执行如下操作
  ```bash
  # 安装Samba服务器
...
- 在虚拟机上执行
    ...
    ![](img/sharedfinal.PNG)
- 下载整个目录\
  （貌似linux和Windows上的使用的用户名必须一样才能下载目录，当前使用的是重复之前操作新建的用户。）
  ![](img/download.PNG)
...
### 6. DNS
- 配置文件: 
  - [/etc/bind/named.conf.options](config/named.conf.options)  
  - [/etc/bind/named.conf.local](config/named.conf.local)
  - [/etc/bind/db.cuc.edu.cn](config/db.cuc.edu.cn)
